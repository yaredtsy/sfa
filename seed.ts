...
export async function seed(connection: Connection) {
  ...
  const user = await (await connection).getRepository(User).create(userData).save();
     
  const nation = await (
    await connection
  ).getRepository(Nation).create({
      nationCode: "ET",
      nationName: "Ethiopia",
      created_by: user,
    }).save();
    const company = await connection.getRepository(Company).create({
        companyCode: 'UB',
    ...
    const region = await connection.getRepository(Region).create({
        regionCode: 'BDR',
        regionName: "Bahir Dar",
        company_id: company,
        created_by: user
    }).save();
    const territory = await connection.getRepository(Territory).create({
        territoryCode: 'LO',
        territoryName: 'Logia',
        region_id: region,
        created_by: user
    }).save();
    const truck = await connection.getRepository(Truck).create({
        truckCode: 'AA-BO-001',
    ...
    const route = await connection.getRepository(Route).create({
        routeCode: 'AA-BO-001-01',
        routeName: 'Around Gerji',
        truck_id: truck,
        created_by: user
    }).save();
    const city = await connection.getRepository(City).create({
        city: 'Addis Ababa',
        subCity: 'Bole',
        specificArea: 'Gerji',
        region_id: region,
    ...
    const channel = await connection.getRepository(Channel).create({
        channelName: 'BAR',
        created_by: user
    }).save();
    const outlet = await connection.getRepository(Outlet).create({
        channel_id: channel,
        city_id: city,
        company_id: company,
        created_by: user,
    ...
    const agent = await connection.getRepository(Agent).create({
        agentName: 'Yeneneh',
        agentCode: 'YT',
        address: 'Gerji',
        company_id: company,
